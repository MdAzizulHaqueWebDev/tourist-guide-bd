...
const RequestToAdminMakeGuider = () => {
	const { user, loading } = useAuth();
	const axiosSecure = useAxiosSecureInstance();
	const { roleLoading } = useRole();
	if (loading || roleLoading) return <Spinner />;
	const handleRequestBtn = async () => {
		const { data } = await axiosSecure.patch(`/status/${user?.email}`, {
			status: "requested",
		});
		if (data.modifiedCount) {
			toast.success("Requested Success");
		} else {
			toast.error(" Already requested Pls wait for admin approval");
		}
	};
	return (
		<>
			<SectionTitle heading={"Send a Request For Make Guider"} />
			<section className="flex justify-center mx-2">
				<div className="mockup-phone lg:hidden  mx-auto">
					<div className="camera"></div>
					<div className="display">
						<div className="artboard artboard-demo phone-1">
							<div className="text-center space-y-3">
								<p className="text-xl font-medium">
									Are You Interested To Be Guider ?
								</p>
								<button className="btn bg-[#89ABE3] btn-sm">Request Now</button>
							</div>
						</div>
					</div>
				</div>
			</section>
			<div className="mockup-browser border bg-base-300 hidden mx-5 pt-0 lg:block">
				<div className="mockup-browser-toolbar">
					<div className="input">https://tourguiderbd.com</div>
				</div>
				<div className="flex justify-center px-4 py-16 bg-base-200">
					<div className="text-center space-y-3">
						...
						<button
							...
							className="btn bg-[#89ABE3] btn-wide btn-sm"
						>
							Request Now
						</button>
					</div>
			...
			<div className="bg-blue-100 mx-1 rounded mt-5">
				<section
					id="features"
				...
				>
					<div className="relative mx-auto max-w-5xl text-center">
						...
						<h2 className="block w-full bg-gradient-to-b from-white to-gray-400 bg-clip-text font-bold text-transparent text-3xl sm:text-4xl">
							Guide a tourist That Your Customers Love
						</h2>
					...
					<div className="relative mx-auto max-w-7xl z-10 grid grid-cols-1 gap-10 pt-14 sm:grid-cols-2 lg:grid-cols-3">
						<div className="rounded-md border border-neutral-700 bg-neutral-900/50 p-8 text-center shadow">
							<div
...
export default RequestToAdminMakeGuider;
